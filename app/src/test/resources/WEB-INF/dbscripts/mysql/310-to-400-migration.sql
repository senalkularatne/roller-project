...
alter table weblogentry drop index we_pubentry_idx;


-- fix wacky indexs which ended up with a size constraint
alter table rag_entry drop index rage_sid_idx;
create index rage_sid_idx on rag_entry(subscription_id);

alter table rag_group_subscription drop index raggs_gid_idx;
create index raggs_gid_idx on rag_group_subscription(group_id);

alter table rag_group_subscription drop index raggs_sid_idx;
...
-- alter table weblogentry drop index index_we_pubtime_idx;
-- alter table roller_comment drop index co_pending_idx;
-- alter table roller_comment drop index co_approved_idx;
