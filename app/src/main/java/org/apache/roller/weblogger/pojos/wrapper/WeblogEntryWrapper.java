...
public final class WeblogEntryWrapper {
    ...
    public List getComments() {
        ...
        String currentTags = this.getTagsAsString();
        ...
        WeblogEntryComment weblogEntryComment;
        String entryCommentAsString;
        
        int i = 0;
        while(it.hasNext()) {
        	weblogEntryComment = (WeblogEntryComment) it.next();
            
        	entryCommentAsString = weblogEntryComment.getContent(); // "the whole comment as a string"
            
            // mark important words
        	String[] commentWordArray = entryCommentAsString.split(" ");
            
        	for (int j = 0; j < commentWordArray.length; ++j) {
            	if (currentTags.contains(commentWordArray[j])) {
            		commentWordArray[j] = "MARK" + commentWordArray[j];
            	}
            }
            
            weblogEntryComment.setContent(String.join("", commentWordArray));
            
            wrappedCollection.add(i,WeblogEntryCommentWrapper.wrap(weblogEntryComment, urlStrategy));
            i++;
        }
    ...
    public List getComments(boolean ignoreSpam,boolean approvedOnly) {
        ...
        while(it.hasNext()) {
            ...
            // mark important words
        	String[] commentWordArray = entryCommentAsString.split("\\s+");
            
        	for (int j = 0; j < commentWordArray.length; ++j) {
            	if (currentTags.contains(commentWordArray[j])) {
            		commentWordArray[j] = "HIGHLIGHTME" + commentWordArray[j];
            	}
            }
