...
alter table website rename to weblog;
...
-- two weblog columns renamed
    alter table weblog add column visible boolean;
    alter table weblog alter visible set default true;
    update weblog set visible=true;
    alter table weblog alter visible set not null;
...
-- some tables renamed
alter table folder rename to bookmark_folder;
alter table rolleruser rename to roller_user;
alter table webpage rename to weblog_custom_template;
...
-- Allow users to order their weblog categories (zero-based)
    alter table weblogcategory add column position integer;
    alter table weblogcategory alter position set default 0;
    update weblogcategory set position=0;
    alter table weblogcategory alter position set not null;

-- Removal of custom ping targets
delete from pingtarget where websiteid is not null;
drop index pt_websiteid_fk;
...
drop index folder_namefolderid_uq;
...
-- Adding blog-specific web analytics (e.g. Google Analytics) tracking code
    alter table weblog add column analyticscode text default null;
...
drop index ref_entryid_fk;
drop index ref_websiteid_fk;
